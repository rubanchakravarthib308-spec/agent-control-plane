...
export class PostgresRunStore implements RunStore {
  ...
  async createRun(run: RunRecord): Promise<void> {
    await this.sql`
      insert into agent_runs (id, goal_id, objective, status, created_at)
      values (${run.id}, ${run.goal.id}, ${run.goal.objective}, ${run.status}, ${run.createdAt})
    `;
  }
  ...
  async getRun(runId: string): Promise<RunRecord | null> {
    const rows = await this.sql`
      select id, goal_id, objective, status, created_at, finished_at
    ...
    return {
      id: String(row.id),
      goal: { id: String(row.goal_id), objective: String(row.objective) },
      status: row.status as RunStatus,
      createdAt: new Date(row.created_at as string | Date).toISOString(),
      finishedAt: row.finished_at ? new Date(row.finished_at as string | Date).toISOString() : undefined,
    };
  }

  async getSteps(runId: string): Promise<StepRecord[]> {
    const rows = await this.sql`
      select run_id, step_id, step_order, action, tool, input, risk, status, approval, output, verification
      from agent_run_steps where run_id = ${runId} order by step_order asc
    `;
    return rows.map((row) => ({
      ...
      step: {
        ...
        risk: row.risk as PlanStep["risk"],
      },
      approval: (row.approval ?? undefined) as ApprovalDecision | undefined,
      output: row.output,
  ...
  async listAudit(runId: string): Promise<AuditEvent[]> {
    const rows = await this.sql`
      select occurred_at, event_type, message, metadata
      from agent_audit_events where run_id = ${runId} order by id asc
    `;
    return rows.map((row) => ({
      at: new Date(row.occurred_at as string | Date).toISOString(),
